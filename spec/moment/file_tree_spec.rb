...
describe Moment::FileTree, :fakefs do
  describe "in a directory" do
  ...
  describe "without a specified directory" do
    let(:files){['tfile']}
    let(:file_list){["#{TEST_DIR_NAME}/tfile"]}
    it "should create a directory with a single file" do
      Moment::FileTree.new(files).build_tree
      files.each {|f| File.exist?(f).should be_true}
    end
  end
end